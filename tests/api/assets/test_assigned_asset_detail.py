...
def test_put_patch_assignee_invalid_data(admin_factory, assignee_factory, authed_token_client_generator):
    ...
    client = authed_token_client_generator(user)
    put_response = client.patch(reverse('assigned-asset-detail', kwargs={'pk': assignee.id}), data=data, format='json')
    patch_response = client.patch(reverse('assigned-asset-detail', kwargs={'pk': assignee.id}), data=data, format='json')
...
def test_patch_assignee_non_admin(user_factory, assignee_factory, asset_factory, authed_token_client_generator):
    ...
    client = authed_token_client_generator(user)
    put_response = client.put(reverse('assigned-asset-detail', kwargs={'pk': assignee.id}), data=data, format='json')
    patch_response = client.patch(reverse('assigned-asset-detail', kwargs={'pk': assignee.id}), data=data, format='json')

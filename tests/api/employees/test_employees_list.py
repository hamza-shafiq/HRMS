...
def test_create_employees(admin_factory, department_factory, authed_token_client_generator):
    ...
    data = {"first_name": 'Usama', "last_name": 'Tariq', "phone_number": +923046369800, "national_id_number": 33,
            ...
            "employee_status": "WORKING", "username": 'usama123', "email": 'usama@gmail.com', "password": "paklove"}
    client = authed_token_client_generator(user)
...
def test_create_employees_invalid_status(admin_factory, department_factory, authed_token_client_generator):
    user = admin_factory()
    department = department_factory()
    data = {"first_name": 'Usama', "last_name": 'Tariq', "phone_number": +923046369800, "national_id_number": '2341223488',
            "emergency_contact_number": 934233800, "gender": "MALE", "department": department.id, "designation": "Developer",
            "bank": "Habib", "account_number": 324244, "profile_pic": "http://asfasf.asd", "joining_date": "1990-06-20 08:03",
            "employee_status": "invalid", "username": 'usama123', "email": 'usama@gmail.com', "password": "paklove"}
    client = authed_token_client_generator(user)
    response = client.post(reverse('employees-list'), data=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST

...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    def employee_detail_with_id(self, request):
        user = request.user
        if user.is_superuser:
            ...
            if emp_id:
                try:
                    ...
                    if record:
                        serializer = EmployeeSerializer(record, many=True)
        ...
        return JsonResponse({'error': 'Only admin can see any employee data'}, status=status.HTTP_401_UNAUTHORIZED)
    ...
    def employee_detail(self, request):
        ...
        if record:
            serializer = EmployeeSerializer(record, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        return JsonResponse({'error': 'This user has no employee data'}, status=status.HTTP_204_NO_CONTENT)
